...
- **Usage statistics** (number of requests scrubbed, count of PII items detected per type) are saved to your browser's extension storage. These stats never leave your browser and can be cleared at any time via the "Clear Statistics" button in the extension popup.
...
- The proxy **host, port, and protocol you enter are stored only on your device** and are never transmitted to us.
- Your traffic to the supported AI sites is routed through that proxy; all other sites continue to use your normal connection.
...
- Statistics stored in browser storage persist across sessions but are cleared when you click "Clear Statistics."
