...
const COMPLIANCE_PROFILES = {
  none: null,
  gdpr: ['email','phone','ip','address','ssn','passport','dob','license','uk-ni','uk-nhs','de-id','de-tax','fr-insee','it-cf','se-pn','cn-id','in-aadhaar','in-pan','br-cpf','br-cnpj','kr-rrn','mx-curp','mx-rfc','ca-sin','jp-my','au-tfn'],
  hipaa: ['email','phone','ssn','dob','address','passport','license','in-aadhaar','in-pan','ca-sin','uk-ni','uk-nhs'],
  'pci-dss': ['cc','bank','routing','apikey','pwd','jwt','aws-key','github','slack'],
  ccpa: ['email','phone','ip','address','ssn','dob','license','passport']
};

function getActiveRules() {
  const config = vscode.workspace.getConfiguration('ai-firewall');
  const profile = config.get('complianceProfile', 'none');
  const ids = COMPLIANCE_PROFILES[profile];
  if (!ids) return PII_RULES;
  return PII_RULES.filter(r => ids.includes(r.id));
}
...
function activate(context) {
  ...
  diagCollection = vscode.languages.createDiagnosticCollection('ai-firewall');
  ...
  context.subscriptions.push(
    vscode.workspace.onDidChangeConfiguration(e => {
      if (e.affectsConfiguration('ai-firewall.complianceProfile') || e.affectsConfiguration('ai-firewall.enabled') || e.affectsConfiguration('ai-firewall.diagnosticsEnabled')) {
        const editor = vscode.window.activeTextEditor;
...
function scanDocument(document) {
  ...
  const config = vscode.workspace.getConfiguration('ai-firewall');
  if (!config.get('enabled', true) || !config.get('diagnosticsEnabled', true)) {
  ...
  for (const rule of rules) {
    ...
    while ((match = regex.exec(text)) !== null) {
      if (rule.luhn) {
        ...
        if (clean.length >= 13 && clean.length <= 19 && !luhnCheck(clean)) continue;
      }
      ...
      const diag = new vscode.Diagnostic(
        range,
        'AI Firewall: Potential ' + rule.name + ' detected (' + rule.label + ')',
        vscode.DiagnosticSeverity.Information
      );
      diag.source = 'ai-firewall';
      diag.code = rule.id;
...
function debouncedScan(document) {
  ...
  debounceTimer = setTimeout(() => scanDocument(document), 400);
}
