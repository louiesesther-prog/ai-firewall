(function() {
  ...
  function contextScore(pattern, text, matchIndex, matchValue, baseConf) {
    ...
    // Check reduce keywords (false positive signals)
    var reduceWords = [];
    ...
    // Token-boundary check: if match is part of a longer alphanumeric token, reduce confidence
  ...
  chrome.storage.local.get(['maskMode', 'isEnabled', 'enableSound', 'blockedSites'], function(r) {
    ...
    // Start response scanner if enabled
    if (isEnabled) {
      ...
      for (var i = 0; i < mutations.length; i++) {
        ...
        for (var j = 0; j < added.length; j++) {
          ...
          if (node.nodeType === 3) {
            ...
            // Also check children with a small delay for dynamic rendering
            (function(n) {
